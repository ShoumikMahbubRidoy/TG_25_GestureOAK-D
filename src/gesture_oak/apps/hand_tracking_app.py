...
def main():
    ...
    try:
        while True:
            ...
            # Mark a processed frame for FPS only when a frame exists
            ...
            cv2.putText(
                frame, f"Hands: {len(hands)}", (10, 60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
            )
            depth_status = "Depth: ON" if depth_frame is not None else "Depth: OFF"
            cv2.putText(
                frame, depth_status, (10, 450),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                (0, 255, 0) if depth_frame is not None else (0, 0, 255),
                2
            )
            ...
            if len(hands) == 0:
                cv2.putText(
                    frame, "IR Mode: Show palm; 80–160 cm",
            ...
            cv2.putText(
                frame, f"Swipes: {stats['total_swipes_detected']}", (10, 90),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
            )
            ...
            if progress:
                ...
                cv2.putText(
                    frame, f"State: {progress['state']}", (10, 120),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, state_color, 2
                )
                if progress['distance'] > 0:
                    cv2.putText(
                        frame, f"Distance: {progress['distance']:.0f}px (need: 80px)", (10, 150),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, state_color, 2
                    )
                    cv2.putText(
                        frame, f"Velocity: {progress['velocity']:.0f}px/s (need: 30-1000)", (10, 180),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, state_color, 2
                    )
                    cv2.putText(
                        frame, f"Progress: {progress['progress']:.1%}", (10, 210),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, state_color, 2
                    )
            ...
            if frame_count - last_swipe_alert < 90:  # ~3s @30fps
                cv2.putText(
                    frame, "SWIPE DETECTED!", (frame.shape[1] // 2 - 100, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 3
                )
                cv2.arrowedLine(
                    frame,
                    (frame.shape[1] // 2 - 50, 80),
                    (frame.shape[1] // 2 + 50, 80),
                    (0, 255, 0), 5
                )
            ...
            for i, hand in enumerate(hands):
                ...
                if frame_count % 2 == 0:
                    # these prints are noisy; keep your original behavior
        ...
        # Always close resources, even on exceptions
        try:
            detector.close()
        except Exception:
            pass
        ...
        # Stop FPS (safe if already stopped / not started)
        if hasattr(detector, "fps_counter") and hasattr(detector.fps_counter, "stop"):
            try:
                detector.fps_counter.stop()
            except Exception:
                pass
        ...
        if hasattr(detector, "fps_counter") and hasattr(detector.fps_counter, "frames"):
            try:
                frames = int(detector.fps_counter.frames())
            except Exception:
                frames = 0
